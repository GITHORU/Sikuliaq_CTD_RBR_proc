...
if __name__ == '__main__':
    ...
    for (i, svp) in enumerate(l_SVP):
        ...
        ax2profile(ax, x_label="Soudspeed (m/s)", y_label="Depth (m)", invert=True, title="SVP n°"+str(i+1), legend=False)
        ax2profile(ax_shal, x_label="Soudspeed (m/s)", y_label="Depth (m)", invert=True, title="SVP n°"+str(i+1)+" (Shallow)", legend=False)
        l_ax.append(ax)
        l_fig.append(fig)
